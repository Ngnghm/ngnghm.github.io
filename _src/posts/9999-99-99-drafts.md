...
1- Pure Functional Reactive language:
  Input signals are files in the version control system, output signals are build artifacts.
  Mapping of namespace to real and virtual filesystems.
  Pull semantics rather than push. (Demand Reactive Programming... see awelonblue)
  Distributed implementation, intent-addressed cache.

2- Dynamic, higher-order control flow:
  The dependency graph is the result from evaluating build files, that themselves change with time.
  Yet, some mapped names may be stable across these changes.
  Yet, some files and artifact may be renamed.
  Usual schema upgrade issues in a typed programming language
  — even if the types may only deal with the names that are available.

3- Staged evaluation model:
  Intermediate outputs are programs that later get evaluated.
  As filesystems are merged into first-class namespaces of the programming language,
  actions merge environments with runtime files and evaluate them,
  possibly creating new (partial) environments.
  Evaluation happens in first-class virtual machines.

4- Dependency detection *is* code coverage at the meta-level.
  Granularity may vary.
  Zoom on demand for debugging:
  Separate code and debug information.
  Code without debug information is more stable facing small changes, so there is more sharing.
  Execute shared code without debugging dependencies if doesn't fail.
  But when things fail, re-run the code with omniscient debugging.
  If needed, run with omniscient debugging at the meta-level, etc.

Conclusion: the regular language is the build system, and the build system is the regular language;
but we have learned more about the structure of the language.
