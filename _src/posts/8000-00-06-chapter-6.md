...
the way Houyhnhnm systems are architected leads to different enough concepts
that for the most part there isn't a direct one-to-one correspondance between
...
modern computer hardware ensures that runtime,
all programs beside the kernel run in a "user mode" that only sees these abstractions,
whereas the kernel alone runs in a "kernel mode" that gives it direct access to the hardware.
The kernel can also use this hardware support to provide low-level isolation between "processes",
allowing multiple user programs to run at the same time while ensuring that
...
The notion of kernel like most concepts of Human computer systems was too artefact-oriented
...
And then I was at loss to distinguish what exactly kinds of interaction a kernel does
...
and makes available to multiple programs, either one after the other, or at the same time;
...
as well does a sound mixer, a 3D scene renderer, a Window system,
...
that cannot possibly be reduced to a fixed set of resources,
but is an irreducible aspect of most programs.
...
or some indirect way of naming a resource such that
...
or emulating it painfully with monads.
...
at which point you can only access those files owned by said user,
...
or become a big security liability whenever it's shared in any way.
...
even less so one that's so simple that it maps directly to a handful hardware operations,
or to some ad-hoc data structure.
...
based on libraries can express all the usual security paradigms and
...
all programs that run "on top of" the kernel are _user_ program,
and their computations are restricted to _only_ be combinations
of the primitives provided by the _system_.
...
— and when properly done, it also
...
and this in turn allows to keep software and hardware development _modular_,
to divide the labor in having to master the otherwise unsurmountable complexity
of a complete computer system.
...
by using an otherwise opaque API, programmers who use the library or service
do not have to worry how things are implemented underneath,
...
and not even have to worry about following it manually:
...
but also acknowledge that it is not reserved to a one single "system" abstraction
...
Their program algebras thus have a notion of first-class programming system
(which includes programming language as well as programming runtime),
that programmers can freely define as well as use,
in every case providing abstraction, rather than forcing the
...
The continued respect of invariants that matter to subsystems is thus a pure matter of convention,
that goes wholly unenforced by the kernel, but only by myriads of informal naming conventions,
...
and a lot setup indeed.
Still, they exist and with them, companies with a large expensive engineering crew
can enforce their company-wide invariants,
and enjoy the relative simplicity that comes when you can reason about
the entire system, knowing that parasitic behaviors have been eliminated,
...
are those that underlie any "base" system, but those of the overall system.
To them, the idea that there should be a one privileged "base" system,
with a kernel that possesses a monopoly on invariant enforcement,
what more enforcement of invariants way too low-level to matter to any user,
...
What more, Houyhnhnms understand that
not only the invariants that matter are almost never those of a "base" system,
but those you want to build from are almost never those of a low-level machine;
instead, just they have first-class notions for computing systems
...
keeping the overall system simpler, easier to reason with, and easier to implement efficiently.
...
In the end, it appeared to Ngnghm that once again,
...
Humans think in terms of fixed artefacts that they can sell;
Houyhnhnms think in terms of given or evolving computing processes.
...
in a fluid set that changes as new subsystems are started or modified.
