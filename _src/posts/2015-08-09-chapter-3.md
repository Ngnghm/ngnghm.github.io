...
separated by slashes `/` (or on some systems, blashslashes `\`, or something else).
...
and would require ad hoc containers or protocol generators to allow larger objects to contain smaller ones,
...
— while preserving all user-visible identities and relationships across such changes in representation.
...
An "access path" is thus a natural notion in all computing systems <!-- see Clojure: assoc-in, update-in -->
...
For instance, you might adopt a _model-view-controller_ approach,
and consider the view as transient while only logging changes to the controller, or even only to the model;
...
Now, beyond clutter that useless filly up memory,
...
you need an _external_ system that can stop the other system, inspect it, fix it, and restart it.
...
But virtualization doesn't mean the same thing in a Houyhnhnm computing system as in a Human computer system.
In a Human computer system, virtualization is an _ad hoc_ tool for system administrators
allowing the deployment of specially prepared servers by faking an entire physical computer
...
it is available at whichever level of abstraction the users and programmers specify their computations,
...
Aternatively, when bad things happen in said official reality,
they can be [retcon](http://www.jargon.net/jargonfile/r/retcon.html)'ed into having been but a bad dream,
as the bad reality is demoted into being a mere unmerged error branch
while a nicer reality becomes the master copy.
Of course, it may be too late to undo bad communications with external systems, if they were let happen;
...
the user can dynamically re-wire all these connections from a monitor outside the virtualized system,
...
Houyhnhnms think of code and data as coming together, part of the same interaction with the Sentient user,
with data and code being useless without the other, or out of synch with the other;
and thus Houyhnhnm computing systems casually apply version control to the entire state of the system.
...
ultimately, it cannot be separated from the rest.
