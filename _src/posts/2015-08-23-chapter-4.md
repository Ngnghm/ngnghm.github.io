...
which allows them to enjoy qualities not found in Human computer systems that can't and don't.
...
programs can only manipulate an object
if they get the entire large assemblage of libraries just right,
...
of arguments that are as many points where attackers may inject unwanted behavior;
...
consider the software development and use processes as primary, not the artefacts,
...
and comes for free to users and programmers;
they don't have to manually flush mass storage buffers any more
than they have to manually flush memory cache lines or to manually spill registers to memory.
...
I already mentioned in [the previous chapter](chapter-3-the-houyhnhnm-version-of-salvation/)
...
(and/or to use some very slow interpreter or very rigid virtual machine).
...
etc.
Of course, these implementation strategies may of course reject requests to create or move processes
in a domain where some incompatibility exists:
the program might not pass some static type checks,
or possess appropriate permissions, or sufficient resources, etc.
Then the user or programmer may have to try a different strategy or modify his program.
...
Of course, either when you start the program or later, you may decide to constrain the process
...
Formally speaking, if you wrote a program in abstract language A,
and specify a given implementation I of language A generating code in concrete language C,
then you actually specified a program in language C.
And as long as you don't proceed to make modifications at the lower level of language C
that invalidate the abstraction to language A, then you can remove the constraint,
go back to the level of program A,
and later choose a different implementation I' targetting language C'.
...
humans are just so much worse than machine (not to mention slower and more expensive)
at automating the implementation of strategies (which of course goes against the
[Sacred Motto of the Guild of Houyhnhnm Programmers](/blog/2015/08/03/chapter-2-save-our-souls/)).
(Of course, quality is even worse when the underlying design patterns have not even
been recognized and their properties semi-formally analyzed.)
Errors can of course be made when writing the strategy automation —
...
what more, it's much easier to keep it right when the pattern or its parameters change,
and maintenance requires the millions of manual instance to be modified accordingly.
As said Rich Hickey, _(Design) patterns mean "I have run out of language."_
...
All computer software has to start from a given base:
...
the operating system itself can be considered such a tower of layers, on top to build higher towers.
...
Because it is actually important to share data between different programs,
Human software developers onerously build _abstractions_, without system support,
...
For an extreme case, some Common Lisp programs reimplement Common Lisp in Common Lisp
to add first-class continuations, optionally serializable ones,
...
This ability to build _down_ as well as up crucially relies
...
just like the turtles of the common joke, whereby there are turtles _all the way down_;
